/*
 * BLF-A6-Mod firmware (special-edition group buy light)
...
#if (ATTINY == 13)
#define DELAY_TWEAK         835  // Real FCPU for Testedandbaked's chip = 835/1200*4.8MHz = 3.34MHz
#elif (ATTINY == 25)
...
// output to use for blinks on battery check mode + egg timer mode (primary PWM level, alt PWM level)
// Use 20,0 for a single-channel driver or 0,20 for a two-channel driver
#define BLINK_BRIGHTNESS    0,20
// Quick blink brigtness for egg timer mode
#define QBLINK_BRIGHTNESS    0,6

// Modes
#define NUM_MODES1          6
// PWM levels for the big circuit (FET or Nx7135)
#define MODESNx1            0,0,5,7,90,255
// PWM levels for the small circuit (1x7135)
#define MODES1x1            4,20,70,255,255,0
...
#define MODES_PWM1          PHASE,FAST,FAST,FAST,FAST,PHASE

// Hidden modes are *before* the lowest (moon) mode, and should be specified
// in reverse order.  So, to go backward from moon to turbo to strobe to
// battcheck, use BATTCHECK,STROBE,TURBO.
#define NUM_HIDDEN          6
#define HIDDENMODES         EGG,SOS,BATTCHECK,BIKING_STROBE,TURBO,STROBE
...
// Uncomment to enable SOS Mode
#define SOS  251			// Convenience code for SOS mode
// Uncomment to enable egg timer mode
#define EGG  250			// Convenience code for egg timer mode
...
/*
// 100-75-50-25-0% voltage table
// These values were measured using wight's "A12HYBRID-S" driver built by Testedandbaked.
// Your mileage may vary.
#define ADC_100         194 // the ADC value for 100% full (4.2V resting)
#define ADC_75          185 // the ADC value for 75% full (4.0V resting)
...
#define ADC_CRIT        125 // When do we shut the light off (2.7V)
*/

// 10% Voltage Table
// HKJ's 0.1a battery drain tests were used to calculate the correct voltage to % conversion for efest 10440
// The ADC values below were extrapolated using the above measured values.
// For efest 10440, you can use use the listed voltages, but the ADC values you'll need to calculate.
// http://lygte-info.dk/review/batteries2012/Efest%20IMR10440%20V1%20350mAh%20%28Black%29%20UK.html

#define ADC_100			194 //4.20v
#define ADC_90			188 //4.07v
#define ADC_80			183 //3.97v
#define ADC_70			180 //3.90v
#define ADC_60			176 //3.82v
#define ADC_50			175 //3.795v
#define ADC_40			174 //3.77v
#define ADC_30			172 //3.72v
#define ADC_20			171 //3.695v
#define ADC_10			169 //3.66v
#define ADC_0			138 //3.00v
...
#ifdef OFFTIM3
#define CAP_SHORT           200  // Anything higher than this is a short press (0.5s)
#define CAP_MED             94  // Between CAP_MED and CAP_SHORT is a medium press (1.5s)
...
PROGMEM const uint8_t modesNx[] = { MODESNx1, HIDDENMODES };
PROGMEM const uint8_t modes1x[] = { MODES1x1, HIDDENMODES_ALT };
PROGMEM const uint8_t modes_pwm[] = { MODES_PWM1, HIDDENMODES_PWM };

/*
// For 100-75-50-25-0% voltage mode
PROGMEM const uint8_t voltage_blinks[] = {
    ADC_0,    // 1 blink  for 0%-25%
    ADC_30,   // 2 blinks for 25%-50%
    ADC_50,   // 3 blinks for 50%-75%
    ADC_80,   // 4 blinks for 75%-100%
...
// For 10% voltage mode
PROGMEM const uint8_t voltage_blinks[] = {
	ADC_0,    // 1 blink  for 0%-10%
	ADC_10,   // 2 blinks for 10%-20%
	ADC_20,   // 3 blinks for 20%-30%
	ADC_30,   // 4 blinks for 30%-40%
	ADC_40,   // 5 blinks for 40%-50%
	ADC_50,   // 6 blinks for 50%-60%
	ADC_60,   // 7 blinks for 60%-70%
	ADC_70,   // 8 blinks for 70%-80%
	ADC_80,   // 9 blinks for 80%-90%
	ADC_90,   // 10 blinks for 90%-100%
	ADC_100,  // 11 blinks for >100%
	255,      // Ceiling, don't remove
};
...
// Slow blink (for battery voltage readout, egg timer)
...
// Quick double blink every 15s (for egg timer)
void qblink(uint8_t val2)
{
	for(; val2>0; val2--)
	{
	
			set_output(QBLINK_BRIGHTNESS);
			_delay_ms(50);
			set_output(0,0);
			_delay_ms(100);
			set_output(QBLINK_BRIGHTNESS);
...
//Blinky modes definitions start here
#ifdef STROBE
        else if (output == STROBE) {
            // 10Hz tactical strobe With Turbo
            set_output(255,0);
            _delay_ms(50);
            set_output(0,0);
            _delay_ms(50);
            ...
            // normal version // brightness (0,100) = 1.7h on efest 10440
            for(i=0;i<4;i++) {
                set_output(255,0);
                _delay_ms(5);
                set_output(0,100);
	...
	// SOS Signal
	// Brightness of (0,100) = 10h on efest 10440
	for(i=0;i<3;i++) {
		for(j=0;j<3;j++) {
		set_output(0,100);
		if(i==1) {
	...
	// 10 minute looping egg timer
	// Slow blinks = minutes elapsed up to 10 e.g. 3 slow blinks = 3:00
	// Quick faint double blinks every 15s e.g. at 1:15, 1:30, 1:45
	for(i=0;i<10;i++) {
		blink(i);
		_delay_ms(15000-500*i);
		qblink(3);
	}
}
#endif // ifdef EGG
            ...
            // blink zero to five times to show voltage (or 0 to 11 times for 10% mode)
            // (~0%, ~25%, ~50%, ~75%, ~100%, >100%)
			blink(i);
        ...
        if (ADCSRA & (1 << ADIF)) {  // if a voltage reading is ready
            ...
            if (lowbatt_cnt >= 2) {
                ...
                _delay_s();  // this will interrupt blinky modes
            }
